...
- Achieve **80% unit test coverage** using **Jest**.

## UI/UX Consistency
- Apply **Tailwind CSS** with `#9844fc` for buttons, links, and highlights; use `dark:` classes for light/dark mode.
- Use **ShadCN** components and **Lucide React** icons for unified UI.
- Support **light/dark mode** with `prefers-color-scheme` detection.
- Ensure **WCAG 2.1 Level AA** accessibility (contrast ratio > 4.5:1 for `#9844fc`).
...
- Optimize **Firebase** queries for free-tier limits.
...
- Maintain a developer **README** with setup instructions and **Firebase** config.
- Document **API endpoints** and **Firebase Functions**.
- Provide **onboarding guides** for new developers.

## Additional Requirements
- Use **Next.js 14** (App Router) with **TypeScript**.
- Style with **Tailwind CSS**, using `#9844fc` (e.g., `bg-[#9844fc]`, `text-[#9844fc]`, `hover:bg-[#7b33cc]`), with `dark:` classes.
...
- Use **Firebase Security Rules** for GDPR/CCPA compliance and role-based access.